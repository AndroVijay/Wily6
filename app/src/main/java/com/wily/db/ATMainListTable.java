...
public class ATMainListTable {

	public static final String TABLE_NAME = "mMainList";
	...
	public ATMainListTable(AppDb appDb) {
		this.appDb = appDb;
	}
	...
	public ArrayList<ATItem> checkAndGetData(Context context) {
		if (isDataPresent()) {
			...
			deleteAllData();
			insertData(getFirstList(context));
			return getAllData();
		}
	}
	...
	public static Vector<ATItem> getFirstList(Context context) {
		...
		item = new ATItem();
		item.setItemActionName(ATUtils.ACTION_LOCK);
		...
		item = new ATItem();
		item.setItemActionName(ATUtils.ACTION_HOME);
